...
class ProtoWriter:
    ...
    def writeInt64(self, int64):
        bs = int64.to_bytes(8, byteorder='little', signed=True)
        self.write(bs)
...
class ProtoBuf:
    ...
    def toJson(self) -> dict:
        """Convert entire ProtoBuf to a clean JSON-friendly dict."""
        seen = set()
        def _gather(fields):
            result = {}
            for f in fields:
                ...
                if f.type == ProtoFieldType.STRING:
                    try:
                        inner = ProtoBuf(f.val)
                        inner_fields = inner.fields[:]
                        fkey = (idx, tuple((ff.idx, ff.type) for ff in inner_fields))
                        if fkey in seen:
                            continue
                        seen.add(fkey)
                        sub = _gather(inner_fields)
                        if sub:
                            if idx in result:
                                if not isinstance(result[idx], list):
                                    result[idx] = [result[idx]]
                                result[idx].append(sub)
                            else:
                                result[idx] = sub
                            continue
                    except Exception:
                        pass
                    val = f.val.decode('utf-8', errors='replace') if isinstance(f.val, bytes) else str(f.val)
                    if idx in result:
                        if not isinstance(result[idx], list):
                            result[idx] = [result[idx]]
                        ...
                        result[idx] = val
                elif f.type in (ProtoFieldType.VARINT, ProtoFieldType.INT32, ProtoFieldType.INT64):
                    val = f.val
                    if idx in result:
                        if not isinstance(result[idx], list):
                            result[idx] = [result[idx]]
